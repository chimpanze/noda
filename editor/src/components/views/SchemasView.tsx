...
export function SchemasView() {
  ...
  const onVisualChange = useCallback((content: Record<string, unknown>) => {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      showToast({
        type: "success",
        message: `Schema "${selected.path}" saved`,
      });
      await loadFiles();
      await reload();
    } catch (err) {
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      ...
      showToast({
        type: "success",
        message: `Schema "${selected.path}" deleted`,
      });
      setSelected(null);
      setEditorValue("");
      setOriginalValue("");
  ...
  const handleCreate = useCallback(async () => {
    ...
    if (!name) return;
    const folder =
      newCategory === "custom" ? customCategory.trim() : newCategory;
    ...
    try {
      const content = {
        [name.replace(/\.json$/, "")]: { type: "object", properties: {} },
      };
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      <ViewHeader
        title="Schemas"
        subtitle="Shared JSON Schema definitions for validation and migration generation"
      />
              ...
              <div className="flex-1 min-h-0 overflow-auto">
                {editorMode === "visual" && parsedContent ? (
                  <SchemaPropertyEditor
                    content={
                      parsedContent as Record<string, Record<string, unknown>>
                    }
