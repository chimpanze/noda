...
import { X, ChevronDown, ChevronRight, AlertTriangle, CheckCircle } from "lucide-react";
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        setEnvFileContent(typeof envData === "string" ? envData : JSON.stringify(envData));
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        {envVars.length > 0 && (
          <div className="border-t border-gray-200 pt-4">
            ...
            >
              {envExpanded ? <ChevronDown size={14} className="text-gray-400" /> : <ChevronRight size={14} className="text-gray-400" />}
              ...
              <div className="mt-3 space-y-4">
                ...
                {envVars.some((v) => !v.defined) && (
                  <div className="p-2 bg-amber-50 border border-amber-200 rounded text-xs text-amber-800">
                    Some environment variables referenced in config are not defined.
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="bg-gray-50 border-b border-gray-200">
                        <th className="text-left px-3 py-1.5 font-medium text-gray-500">Variable</th>
                        <th className="text-left px-3 py-1.5 font-medium text-gray-500 w-20">Status</th>
                        <th className="text-left px-3 py-1.5 font-medium text-gray-500">Source</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100">
                      {envVars.map((v) => (
                        <tr key={v.name} className={v.defined ? "" : "bg-amber-50/50"}>
                          <td className="px-3 py-1.5 font-mono text-gray-800">{v.name}</td>
                      ...
                      <button
                        onClick={async () => {
                          ...
                          try {
                            ...
                            showToast({ type: "success", message: ".env file saved" });
                            await reload();
                          } catch (err) {
                            showToast({ type: "error", message: `Failed to save .env: ${err}` });
                          } finally {
                            setSavingEnv(false);
                          }
                        }}
                      ...
                      onClick={async () => {
                        try {
                          const template = envVars
                            ...
                            .map((v) => `${v.name}=`)
                            .join("\n");
                          await api.writeFile(".env", template || "# Environment variables\n");
                          showToast({ type: "success", message: ".env file created" });
                          setEnvFileContent(template || "# Environment variables\n");
                          setEnvFileExists(true);
                        } catch (err) {
                          showToast({ type: "error", message: `Failed to create .env: ${err}` });
                        }
                      }}
