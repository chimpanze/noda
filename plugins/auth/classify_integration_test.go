...
//
// This used to need its own fixture, because newTestDB opened SQLite with
// modernc's "_pragma=foreign_keys(1)" — a parameter mattn/go-sqlite3 silently
// ignores — and so ran with foreign keys off (#423). newTestDB now enables
// them for real and asserts the pragma before handing the DB back, so this
