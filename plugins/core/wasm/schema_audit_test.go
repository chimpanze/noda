...
func TestConfigSchemasMatchExecutors(t *testing.T) {
	tests := []struct {
		...
		{"wasm.query", (&queryDescriptor{}).ConfigSchema(),
			map[string]any{"data": map[string]any{"query": "get_state"}}, false,
			map[string]any{"data": map[string]any{"query": "get_state"}, "timeout": 5}},
		{"wasm.send", (&sendDescriptor{}).ConfigSchema(),
			map[string]any{"data": map[string]any{"action": "player_move"}}, false,
			map[string]any{"data": map[string]any{"action": "player_move"}, "unexpected": true}},
	}
