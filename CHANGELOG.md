...
### Changed
- Caller-triggerable SQL failures now map to typed errors instead of a blanket 500 (#403). A new `internal/dberr` package classifies Postgres SQLSTATEs and SQLite extended result codes: constraint violations become 409/422, serialization failures and deadlocks 503, statement timeouts 504. **Behavior change:** endpoints that previously returned 500 for these now return 4xx/503/504, so a caller treating any 5xx as retryable will see different statuses. Class 42 errors (undefined column/table/function) deliberately stay 500 — those are author bugs, not caller faults. Unmapped codes are unchanged.
- `api.ValidationError`, `api.ConflictError`, and `api.TimeoutError` gain an optional `Cause` field and `Unwrap` method, matching `api.ServiceUnavailableError` (#403). Additive — existing struct literals are unaffected. `TimeoutError.Error()` deliberately does not render the cause, because it is sent to prod clients unconditionally.
