...
### Fixed
- `make test` / `make test-coverage` no longer sweep in Go code vendored inside `editor/node_modules` (the npm package `flatted` ships a Go implementation under the module root). That third-party 0%-covered package skewed local coverage ~0.9pp below CI's figure; CI avoided it only by accident of step ordering (coverage runs before `make build` triggers `npm install`). Local and CI now both report 81.5%.
