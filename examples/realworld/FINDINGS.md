...
| ~~No slugify/kebab-case expression function in `internal/expr`~~ — **resolved**: `slugify(s)` now exists. This workflow still builds the slug in SQL, because the uniqueness suffix comes from `nextval('articles_id_seq')` inside the same `INSERT`; that is a uniqueness concern, not a missing-function one | `workflows/create-article.json` — slug generation had to be pushed into raw SQL instead of a workflow expression | [#437](https://github.com/chimpanze/noda/issues/437) |
