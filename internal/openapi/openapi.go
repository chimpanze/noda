// Package openapi generates an OpenAPI 3.1 spec from a resolved Noda config.
// It is a pure, framework-agnostic transform: no HTTP wiring lives here.
package openapi
...
// Generate creates an OpenAPI 3.1 spec from the resolved config.
func Generate(rc *config.ResolvedConfig) (*openapi3.T, error) {
	doc := &openapi3.T{
		OpenAPI: "3.1.0",
		Info: &openapi3.Info{
	...
	// Build component schemas from the ref-name-keyed registry so that
	// component names line up with $ref targets (schemas/User -> "User").
	if len(rc.SchemaRegistry) > 0 {
		doc.Components = &openapi3.Components{
			Schemas: make(openapi3.Schemas),
		}
		for refName, schema := range rc.SchemaRegistry {
			schemaRef, err := convertSchema(schema)
			if err != nil {
				continue // skip invalid schemas
			}
			doc.Components.Schemas[strings.TrimPrefix(refName, "schemas/")] = schemaRef
		}
	}
...
// ScalarHTML renders the Scalar API-reference page pointing at specPath.
func ScalarHTML(specPath string) string {
	return `<!DOCTYPE html>
<html>
<head>
