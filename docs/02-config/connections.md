...
5. Binary (non-UTF-8) payloads — e.g. msgpack frames pushed by a Wasm module — are transported base64-encoded in a v2 envelope and delivered byte-exact on remote instances. UTF-8 payloads keep the v1 plain-string envelope. During a mixed-version rolling deploy, instances older than v2 drop binary-payload envelopes with a warning; text/JSON traffic is unaffected.
