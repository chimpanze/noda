...
| `expression_strict_mode` | boolean | `false` | When `true`, undefined variables in expressions produce compile errors instead of silently returning nil. Catches typos like `{{ auth.is_admim }}` at load time. Strict mode validates top-level names against a single shared namespace list, so transport names (`body`, `query`, `params`, `headers`, `request`, `raw_body`, `method`, `path`, `message`, `schedule`) compile in any expression context and resolve to `nil` outside their trigger context; unknown names are still rejected. |
