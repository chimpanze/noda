...
>
> **The reverse does hold:** `noda test` validates the project before it runs anything. It performs the same checks as `noda validate` — node config schemas, service slot references, service config schemas, edge outputs, unwired outcome outputs, middleware builds, workflow graph compilation, cron specs, and worker concurrency limits — and stops with those errors without executing a single test. A project that cannot boot cannot be tested.
